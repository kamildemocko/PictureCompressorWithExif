...
def process_image(root: str, filename: str) -> str:
    ...
    try:
        ...
        os.rename(original_file_path, os.path.join(root, filename))

    return converted, detail, output_path
...
for root, _, files in walk:
    ...
    for file_ in files:
        ...
        converted, detail, output_path = process_image(root, file_)
        ...
        if converted:
            ...
            print(
                f"Error converting file {file_} - original kept, "
                f"detail: {detail}%"
            )
